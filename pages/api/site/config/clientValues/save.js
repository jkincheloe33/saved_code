...
export default async (req, res) => {
  ...
  if (inserted) {
    let insertRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        INSERT INTO clientValues SET ?
      `,
      params: [protectEdit(inserted, { accountId: req.clientAccount.id })],
    })

    recordAuditTrail(req.session.userId, 'insert', 'clientValues', insertRes.insertId, protectEdit(inserted))
    ...
    if (updateRes.changedRows === 1) {
      recordAuditTrail(req.session.userId, 'update', 'clientValues', updated.id, protectEdit(updated))
    }

    res.json({ success: updateRes.changedRows === 1 })
  } else if (deleted) {
    // Not implemented until we get cpc links...EK
    res.json({ success: false, msg: 'Not Implemented until cpc links are implemented' })

    // // Execute delete statement...EK
    // let deleteRes = await wambiDB.executeNonQuery({
    //   commandText: /*sql*/ `
    //     DELETE FROM clientValues
    //     WHERE id = ?
    //       AND accountId = ?
    //     `,
    ...
    //   recordAuditTrail(req.session.userId, 'delete', 'clientValues', deleted.id, protectEdit(deleted))
    // }
...
const allowEdit = ['name', 'description', 'rank']
