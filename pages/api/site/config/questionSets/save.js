...
export default async (req, res) => {
  ...
  if (inserted) {
    let insertRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        INSERT INTO questionSets SET ?
      `,
      params: [protectEdit(inserted, { accountId: req.clientAccount.id })],
    })

    recordAuditTrail(req.session.userId, 'insert', 'questionSets', insertRes.insertId, protectEdit(inserted))
    ...
    if (updateRes.changedRows === 1) {
      recordAuditTrail(req.session.userId, 'update', 'questionSets', updated.id, protectEdit(updated))
    }

    res.json({ success: updateRes.changedRows === 1 })
  } else if (deleted) {
    // Verify that the questionSet is unused.  If not don't allow the delete...EK

    // // Execute delete statement...EK
    let deleteRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        DELETE FROM questionSets
        WHERE id = ?
          AND accountId = ?
      `,
    ...
    if (deleteRes.affectedRows === 1) {
      recordAuditTrail(req.session.userId, 'delete', 'questionSets', deleted.id, protectEdit(deleted))
    }
...
const allowEdit = ['name', 'description']
