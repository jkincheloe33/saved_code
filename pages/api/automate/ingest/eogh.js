...
export default async (req, res) => {
  ...
  try {
    ...
    if (clientAccount.host.split('.')[0] !== 'eogh') {
      return res.json({ msg: 'Incorrect client account DNS for EOGH ingestion.  Check the host in your request.', success: false })
    }

    // Get latest import file...EK
    const importFile = await getIngestFileKey('sftp-eogh', 'xlsx')
    ...
    const fileSources = await parseIngestSourceFiles([
      {
        configKey: 'configs/eogh-config.json',
