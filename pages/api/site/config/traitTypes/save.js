...
export default async (req, res) => {
  ...
  if (inserted) {
    let insertRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        INSERT INTO traitTypes SET ?
      `,
      params: [protectEdit(inserted, { accountId: req.clientAccount.id })],
    })

    recordAuditTrail(req.session.userId, 'insert', 'traitTypes', insertRes.insertId, protectEdit(inserted))
    ...
    if (updateRes.changedRows === 1) {
      recordAuditTrail(req.session.userId, 'update', 'traitTypes', updated.id, protectEdit(updated))
    }

    res.json({ success: updateRes.changedRows === 1 })
  } else if (deleted) {
    // Verify that the group type is unused.  If not don't allow the delete...EK
    let traitsByType = await wambiDB.querySingle({
      queryText: /*sql*/ `
        SELECT COUNT(id) AS traitCount FROM traits WHERE traitTypeId = ?
      `,
      params: deleted.id,
    })

    if (traitsByType.traitCount > 0) {
      return res.json({ success: false, msg: `Trait type used by ${traitsByType.traitCount} trait records` })
    }

    // Execute delete statement...EK
    let deleteRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        DELETE FROM traitTypes WHERE id = ? AND accountId = ?
      `,
    ...
    if (deleteRes.affectedRows === 1) {
      recordAuditTrail(req.session.userId, 'delete', 'traitTypes', deleted.id, protectEdit(deleted))
    }
...
const allowEdit = ['name', 'isReviewFilter']
