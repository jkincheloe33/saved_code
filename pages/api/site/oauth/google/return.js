...
const authorityTokenUrl = process.env.GOOGLE_TOKENURL
...
export default async (req, res) => {
  ...
  try {
    ...
    let body = await fetch(`${authorityTokenUrl}`, {
      method: 'POST',
      headers: {
    ...
    const detailedMessage = error ? error.message : 'Google response error'

    const clientMessage = error?.showOnClient ? detailedMessage : 'Google response error'
