...
export default async (req, res) => {
  ...
  try {
    ...
    if (clientAccount.host.split('.')[0] !== 'mash') {
      return res.json({ msg: 'Incorrect client account DNS for MASH ingestion.  Check the host in your request.', success: false })
    }

    // Get latest import file...EK
    const importFile = await getIngestFileKey('sftp-mash', 'xlsx')
    ...
    const fileSources = await parseIngestSourceFiles([
      {
        configKey: 'configs/mash-config.json',
    ...
    await generateCostGroupClientIds(ingestScope, 'CONCAT(DFI.groups_site, DFI.groups_deptId)')
    await associatePeopleWithCostGroups(
      ingestScope,
      `IF(IFNULL(DFI.peopleGroups_level, 'Team Member') = 'Team Member', ${GROUP_ACCESS_LEVELS.TEAM_MEMBER}, ${GROUP_ACCESS_LEVELS.GROUP_OWNER})`
    )
    ...
    await cleanUpOldFiles('sftp-mash', [importFile.Key])
