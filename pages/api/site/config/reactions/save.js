...
export default async (req, res) => {
  ...
  if (inserted) {
    let insertRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        INSERT INTO reactions SET ?
      `,
      params: [protectEdit(inserted, { accountId: req.clientAccount.id })],
    })

    recordAuditTrail(req.session.userId, 'insert', 'reactions', insertRes.insertId, protectEdit(inserted))
    ...
    let updateRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        UPDATE reactions SET ? WHERE id = ? AND accountId = ?
      `,
    ...
    if (updateRes.changedRows === 1) {
      recordAuditTrail(req.session.userId, 'update', 'reactions', updated.id, protectEdit(updated))
    }
    ...
    let deleteRes = await wambiDB.executeNonQuery({
      commandText: /*sql*/ `
        DELETE FROM reactions WHERE id = ? AND accountId = ?
      `,
    ...
    if (deleteRes.affectedRows === 1) {
      recordAuditTrail(req.session.userId, 'delete', 'reactions', deleted.id, protectEdit(deleted))
    }
...
const allowEdit = ['name', 'description', 'status']
